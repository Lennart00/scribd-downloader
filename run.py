...
def alphanum_sort(file1, file2):
    ...
    return (alphanum_key(file1) > alphanum_key(file2)) - (
        alphanum_key(file1) < alphanum_key(file2)
    )
...
if not flag_skip:
    with sync_playwright() as playwright:
        ...
        for i in range(1, total_args):
            ...
            books[iter].current_cache_directory = (
                f"{os.getcwd()}/sources/{book_filename}"
            )
            ...
            while True:
                ...
                while True:
                    ...
                    # set page size
                    match = re.findall(
                        "width: ([0-9.]+)px; height: ([0-9.]+)px;", html
                    )[0]
                    ...
                    if page_no == number_of_chapter_pages:
                        merger.write(
                            f"{current_cache_directory}/{books[iter].chapter_no}.pdf"
                        )
                        merger.close()
                        break
...
books = glob.glob(os.path.join(sourcesDirectory, "*"))
# Sort the files
sortedBooks = sorted(books)


# Print the sorted files
for book_iterator in range(0, len(sortedBooks)):
    merger = PdfMerger()
    book_basename = os.path.basename(sortedBooks[book_iterator])
    print("attempting merge of book: {book_basename}")
    bookFiles = glob.glob(os.path.join(sortedBooks[book_iterator], "*"))
    sortedBookFiles = sorted(bookFiles)
    print(book_basename)
    for bookFile in sortedBookFiles:
        print("attempting to merge book {sortedBooks[book_iterator]}")
        merger.append(f"{bookFile}")
        print(bookFile)

    merger.write(f"out/{book_basename}.pdf")
    merger.close()
    # delete cache dir
